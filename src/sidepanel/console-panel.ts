...
const MIN_QUOTE_CANDIDATE_WORDS = 6;

/**
 * Returns the verbatim substring of `pageTextNormalized` that verifies `quote` — either the
 * whole (normalized) quote, or its core with a few boundary words trimmed — or null if it can't
 * be verified at all. Models often lightly trim a quote's edges — dropping a leading connector
 * clause, adjusting trailing punctuation — while keeping the rest character-for-character;
 * requiring an exact whole-string match would reject those as unverifiable even though the
 * substance is a real quote. Trimming is capped so this still can't wave through a paraphrase:
 * most of the quote's words must survive intact and contiguous. The returned substring (rather
 * than just a boolean) is what's searched for on the live page when a citation is clicked, since
 * the model's untrimmed quote may include words that were never actually adjacent on the page.
 */
function findVerifiedCore(pageTextNormalized: string, quote: string): string | null {
  const words = normalizeWhitespace(quote).toLowerCase().split(" ").filter(Boolean);
  if (words.length === 0) return null;
  const full = words.join(" ");
  if (pageTextNormalized.includes(full)) return full;
  ...
  for (let trimStart = 0; trimStart <= MAX_TRIM; trimStart++) {
    for (let trimEnd = 0; trimEnd <= MAX_TRIM; trimEnd++) {
      ...
      if (pageTextNormalized.includes(coreText)) return coreText;
    }
  }
  return null;
}
